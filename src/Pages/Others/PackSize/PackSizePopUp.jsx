...
const initialForm = { packSize: "" };
function PackSizePopUp(props) {
  ...
  useEffect(() => {
    const fetchMaterialCategory = async () => {
      try {
        const data = await getPackSizeById(props.id);
        setForm(data.data);
      } catch (error) {
  ...
  async function onSubmit(e) {
    ...
    if (props.mode === "add") {
      try {
        const data = await createPackSize(form);
        setForm(initialForm);
      ...
      try {
        const data = await editPackSize(form, props.id);
        setForm(initialForm);
        props.onHide(await data);
      } catch (err) {
        setError("Server Error");
